...
/*prize add by zhaopengge, 20210316-start*/
	...
	strcpy(current_tp_info.chip,"Focaltech_FW:0x06");
...
static int fts_spi_parse_dt(struct device *dev, struct fts_ts_data *data)
{
    ...
    //prize add by wangfei for get spi cs suspend and resume pinctrl 20210617 start 
    data->spi_cs_pins_suspend = pinctrl_lookup_state(data->pinctrl, "spi_cs_suspend");
    if (IS_ERR(data->spi_cs_pins_suspend)) {
        ret = PTR_ERR(data->spi_cs_pins_suspend);
        FTS_ERROR("Cannot find touch pinctrl spi_cs_suspend!");
        goto err_out;
    }

    data->spi_cs_pins_resume = pinctrl_lookup_state(data->pinctrl, "spi_cs_resume");
    if (IS_ERR(data->spi_cs_pins_resume)) {
        ret = PTR_ERR(data->spi_cs_pins_resume);
        FTS_ERROR("Cannot find touch pinctrl spi_cs_pins_resume!");
        goto err_out;
    }
    //prize add by wangfei for get spi cs suspend and resume pinctrl 20210617 end
...
static void tpd_suspend(struct device *dev)
{
    ...
    //prize add by wangfei for low tp reset and spi cs when suspend 20210617 start
    tpd_gpio_output(fts_data->pdata->reset_gpio, 0);
    FTS_INFO(" [wolf]make TP spi cs into suspend 1");
    ret = pinctrl_select_state(ts_data->pinctrl, ts_data->spi_cs_pins_suspend);
    if (ret < 0) {
        FTS_ERROR("Set spi cs suspend pin state error:%d", ret);
    }
    FTS_INFO(" [wolf]make TP spi cs into suspend 2");
    //prize add by wangfei for low tp reset and spi cs when suspend 20210617 end
...
static void tpd_resume(struct device *dev)
{
    ...
    //prize add by wangfei for recover TP reset and spi cs when resume 20210617 start
    FTS_INFO(" [wolf]make TP spi cs into resume 1");
    ret = pinctrl_select_state(ts_data->pinctrl, ts_data->spi_cs_pins_resume);
    if (ret < 0) {
        FTS_ERROR("Set spi cs resume pin state error:%d", ret);
    }
    FTS_INFO(" [wolf]make TP spi cs into resume 2");
    tpd_gpio_output(fts_data->pdata->reset_gpio, 1);
    //prize add by wangfei for recover TP reset and spi cs when resume 20210617 end
