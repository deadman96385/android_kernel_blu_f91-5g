	...
	if (pin_state < IMGSENSOR_HW_PIN_STATE_LEVEL_0 ||
		...
		if(sensor_idx == 0) {
			...
			if(ppinctrl_state != NULL && !IS_ERR(ppinctrl_state)) {
				pinctrl_select_state(pinst->ppinctrl, ppinctrl_state);
				pr_debug("prize linchong sensor_idx(%d) pin(%d) pin_state(%d)\n", sensor_idx+3, pin, pin_state);
			}
			else
				PK_PR_ERR("%s : sensor_idx %d fail to set pinctrl, PinIdx %d, Val %d\n", __func__, sensor_idx+3, pin, pin_state);
			mutex_unlock(pinst->pmclk_mutex);
		}
		#endif
