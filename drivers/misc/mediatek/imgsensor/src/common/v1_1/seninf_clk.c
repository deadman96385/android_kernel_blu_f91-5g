					...
					struct ACDK_SENSOR_MCLK_STRUCT *pmclk)
{
	...
	if (pmclk->on) {
		...
		// prize add by linchong 20211108 start
		#ifdef CONFIG_PRIZE_DUAL_CAMERA_ENABLE
		if(pmclk->TG == 1) {
			PK_DBG("[CAMERA SENSOR] prize linchong kdSetSensorMclk on= %d, freq= %d, idx_tg= %d, TG= %d\n",
	       pmclk->on, pmclk->freq, idx_tg, pmclk->TG+1);
			if(clk_prepare_enable(pclk->mclk_sel[idx_tg+1]))
				PK_PR_ERR("[CAMERA SENSOR] failed tg %d\n", idx_tg+1);
			else
			...
			if (clk_prepare_enable(pclk->mclk_sel[idx_freq]))
				...
				atomic_inc(&pclk->enable_cnt[idx_freq]);
			ret = clk_set_parent(pclk->mclk_sel[idx_tg], pclk->mclk_sel[idx_freq]);
		}
		#endif
		// prize add by linchong 20211108 end
	} else {
