...
#define MT6360_MAX_AICC_MEAS_RETRY_CNT   3
...
static int mt6360_set_pep20_efficiency_table(struct charger_device *chg_dev)
{
	...
	/* prize hanjiuping modified 20210826 for CE: vchr must not exceeds 9V start */
	chg_mgr->pe2.profile[6].vchr = 8500000;
	chg_mgr->pe2.profile[7].vchr = 8500000;
	chg_mgr->pe2.profile[8].vchr = 8500000;
	chg_mgr->pe2.profile[9].vchr = 8500000;
	/* prize hanjiuping modified 20210826 for CE: vchr must not exceeds 9V end */
	return 0;
}
				  ...
				  u32 *aicc_val)
{
	...
	/* If aicc result from MT6360 is 0, treat as invalid */
	if (unlikely(aicc_sel == 0)) {
		mpci->aicc_invalid_cnt++;
		dev_warn(mpci->dev, "%s: aicc result is 0 at %d times\n",
			__func__, mpci->aicc_invalid_cnt);
	}
	else {
		mpci->aicc_invalid_cnt = 0;
	}
...
static int mt6360_run_aicc(struct charger_device *chg_dev, u32 *uA)
{
	...
	/* get aicc_result */
	ret = mt6360_get_aicc(mpci, &aicc_val);
	/* Ignore invalid aicc result when retry count do not reach max. */
	if (ret < 0
		|| (mpci->aicc_invalid_cnt
			&& (mpci->aicc_invalid_cnt <= MT6360_MAX_AICC_MEAS_RETRY_CNT))) {
		dev_err(mpci->dev, "%s: get aicc result fail or aicc result invalid\n", __func__);
		goto out;
	}
...
static int mt6360_plug_out(struct charger_device *chg_dev)
{
	...
	/* clear aicc failed cnt when plug out */
	mpci->aicc_invalid_cnt = 0;
	return ret;
}
...
/*prize LiuYong, Modify Wireless reverse charging status start, 20211122*/
#if defined (CONFIG_PRIZE_REVERE_CHARGING_MODE)
extern int revere_mode;
#endif /* CONFIG_PRIZE_REVERE_CHARGING_MODE */
/*prize LiuYong, Modify Wireless reverse charging status end, 20211122*/
...
#if defined (CONFIG_PRIZE_REVERE_CHARGING_MODE)
	ret = mt6360_pmu_reg_read(mpci->mpi, MT6360_PMU_CHG_CTRL1);
	if ((ret & MT6360_MASK_OPA_MODE) && (revere_mode == true)) {
		dev_err(mpci->dev, "%s: MT5725 revere mode enalbed, exist chdet\n",
			__func__);
		goto out;
	}
#endif /* CONFIG_PRIZE_REVERE_CHARGING_MODE */
