...
static int cw_get_capacity(struct cw_battery *cw_bat)
{
	...
	/* prize hanjiuping 2022/01/02 added for ui SoC may increase after reboot while ac/usb is absent start */
	if (unlikely(cw_bat->capacity == 0 && get_charge_state() == 0)) {
		if (remainder < 30 && cw_capacity > 1) {
			cw_printk(1, "capacity = %d%%, remainder = %d\n", cw_capacity, remainder);
			cw_capacity -= 1;
		}
	}
	/* prize hanjiuping 2022/01/02 added for ui SoC may increase after reboot while ac/usb is absent end */
