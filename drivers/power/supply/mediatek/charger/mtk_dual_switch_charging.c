...
//prize added by sunshuai, wireless charge MT5725   15W soft start, 20200428-end
		...
		if ((mtk_pe20_get_is_enable(info) &&
			...
			/* prize hanjiuping added 20211211 for limit PE+ charger output power start */
			if (mtk_pe_get_is_enable(info) && mtk_pe_get_is_connect(info))
				pdata2->input_current_limit
						= pdata->input_current_limit;
			else
				/* Slave charger may not have input current control */
				pdata2->input_current_limit
						= info->data.ac_charger_input_current;
			/* prize hanjiuping added 20211211 for limit PE+ charger output power end */
