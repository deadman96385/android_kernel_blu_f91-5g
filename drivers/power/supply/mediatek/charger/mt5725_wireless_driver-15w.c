...
struct MT5725_dev {
	...
	atomic_t is_tx_mode; /*prize LiuYong, Modify Wireless reverse chg config, 20211122*/
#endif
...
/*prize LiuYong, Modify Wireless reverse charging config start, 20211122*/
int revere_mode = 0;
...
static DEVICE_ATTR(gettxflag, 0444, gettx_flag_show, NULL);

static ssize_t enabletx_show(struct device *dev, struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d", atomic_read(&mte->is_tx_mode));
}

static ssize_t enabletx_store(struct device* dev, struct device_attribute* attr, const char* buf, size_t count)
{
	int error;
	unsigned int temp;
...
static DEVICE_ATTR(enabletx, 0664, enabletx_show, enabletx_store);
#endif
/*prize LiuYong, Modify Wireless reverse charging config end, 20211122*/
...
int set_rx_vout(uint16_t vout)
{
	...
	pre_vout= temp.value;
	pr_info("mt5725_%s: pre_vout = %dmV, vout = %dmV\n", __func__, pre_vout, vout);
	if (pre_vout == vout) {
		pr_info("mt5725_%s: vout already set or vout set running\n", __func__);
		return -1;
	}
	...
	pr_info("mt5725_%s: set rx vout complete\n", __func__);
	return 0;
}
//prize add by lipengpeng 20210419 start 
...
void MT5725_irq_handle(void) {
    ...
    if(temp.ptr[1] & RXMODE){
        ...
        if (val.value & INT_LDO_ON) {			
            ...
            /* prize hanjiuping 20211111 modified for move rx vout setting from INT_EPP/AFC to INT_LDO_ON start */
            if (mte->charge_protocol == EPP) {
                set_rx_vout(9000);
            }
            else if (mte->charge_protocol == AFC) {
                fast_vfc(9000);
            }
            else {
                pr_info("[%s] current charge protocol is %d, skip setting rx vout\n", __func__, mte->charge_protocol);
            }
            /* prize hanjiuping 20211111 modified for move rx vout setting from INT_EPP/AFC to INT_LDO_ON end */
        }
...
//prize add by lipenpeng 20210419 start   EPP  agreement 9V
			/* prize hanjiuping 20211111 modified for move rx vout setting from INT_EPP/AFC to INT_LDO_ON */
			/*set vout to 9000mv*/
			// set_rx_vout(9000);
...
//prize add by lipengpeng 20210419 end 
    ...
    if(temp.ptr[1] & TXMODE){
        ...
        fclr.value = val.value;
        pr_info("[%s] REG_INTFLAG value:0x%04x\n", __func__, val.value);
/*prize LiuYong, if usb charger close otg start, 20211122*/
#if defined (CONFIG_PRIZE_REVERE_CHARGING_MODE)
        if((val.value & INT_CHIP_DISABLE)&&(revere_mode==1)){
            ...
            set_otg_gpio(0);
            mte->tx_count=0;
            //mt_vbus_reverse_off_limited_current();
            atomic_set(&mte->is_tx_mode,0);
            revere_mode=0;
        }
#endif
/*prize LiuYong, if usb charger close otg end, 20211122*/
        if(val.value == 0){
            pr_info("[%s] There's no interruption here\n", __func__);
            return;
        }
...
/*prize LiuYong, Modify Wireless reverse charging config start, 20211122*/
#if defined (CONFIG_PRIZE_REVERE_CHARGING_MODE)
/*Reverse charging device abnormal, stop reverse charging*/
           atomic_set(&mte->is_tx_mode, 3);
#endif
/*prize LiuYong, Modify Wireless reverse charging config end, 20211122*/
